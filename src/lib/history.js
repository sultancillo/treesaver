 ...
 */
treesaver = treesaver || {};
treesaver.history = treesaver.history || {};
...
require('./storage');
  ...
  if (WITHIN_IOS_WRAPPER) {
    ...
    if (document.location.hash) {
      treesaver.history.current_hash = treesaver.history.getNormalizedHash_();

      // Our hashes always start with the delimiter and have at least another
      // character there
      if (treesaver.history.current_hash[0] === treesaver.history.DELIMITER &&
          treesaver.history.current_hash.length >= 2) {
        // Redirect, stripping the intial delimiter
        // Use location.replace instead of setting document.location to avoid
        // breaking the back button
        document.location.replace(treesaver.history.current_hash.substr(1));
      }
    }
